...
/**
 * @brief Blocks untilan event occurs, the returns it
 *
 * @param node Necessary information about the node
 * @return [ @b int ] Value of the event
 */
int select_event(t_nodeinfo *node)
{
    ...
    if (node->server_tcp_sockfd > 0 && node->self_id != -1)
        FD_SET(node->server_tcp_sockfd, &inputs);
    for (t_int *current = node->int_list; current != NULL; current = current->next_int)
    {
        if (current->int_tcp_sockfd > 0)
            FD_SET(current->int_tcp_sockfd, &inputs);
    }
